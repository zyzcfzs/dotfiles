...
class Keybindings:
    ...
    def create_groups_keys(self):
        modifier      = [GROUPS_KEY]
        swap_modifier = [GROUPS_KEY, MOVEMENT_KEY]
        screen_modifier = [MOVEMENT_KEY]
        move_next = Key(screen_modifier, "Right", lazy.screen.next_group()) 
        move_prev = Key(screen_modifier, "Left", lazy.screen.prev_group()) 
        swap_next = Key(swap_modifier, "Right", Functions.window_to_next_group()) 
        swap_prev = Key(swap_modifier, "Left", Functions.window_to_prev_group()) 
    ...
    def init_keys_groups(self, group_names):
        ...
        for icon in group_names:
            ...
            group_keys += [Key([MOVEMENT_KEY], index, lazy.group[icon].toscreen()), Key(
                [MOVEMENT_KEY, GROUPS_KEY], index, lazy.window.togroup(icon, switch_group=True))]
...
class Mouse:
    ...
    def init_mouse(self):
        mouse = [
            Drag([self.mod,SHIFT], "Button1", lazy.window.set_position_floating(),
                 start=lazy.window.get_position()),
            Drag([self.mod,SHIFT], "Button3", lazy.window.set_size_floating(),
                 start=lazy.window.get_size()),
            Click([self.mod,SHIFT], "Button2", lazy.window.bring_to_front())
        ]
        return mouse
