...
if [[ "$(which pacman >/dev/null; echo -n $?)" == "0" ]]; then 
    sudo pacman -Syyu && sudo pacman --needed -S fd xclip nodejs npm alacritty neovim curl wget aria2 tmux zsh ripgrep bat zoxide fzf exa picom nitrogen yarn && common_set_up
