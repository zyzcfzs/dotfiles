...
if [[ "$(which pacman >/dev/null; echo -n $?)" == "0" ]]; then 
    sudo pacman -Syyu && sudo pacman --needed -S fd xclip nodejs npm alacritty neovim curl wget aria2 tmux zsh ripgrep bat zoxide fzf exa picom nitrogen yarn
elif [[ "$(which apt >/dev/null; echo -n $?)" == "0" ]]; then 
    sudo apt update && sudo apt dist-upgrade -y && sudo apt install -y fd-find xclip nodejs npm zsh tmux curl wget aria2 fzf bat ripgrep neovim tmux exa picom nitrogen zoxide
